...
# print(f"Using Voyage AI cloud reranker: {VOYAGE_RERANK_MODEL}")
...
#             # Group pairs by query so each Voyage rerank call reranks one query's documents
#             query_pair_indices: dict = {}
#             for i, (uq, _content) in enumerate(all_pairs):
#                 query_pair_indices.setdefault(uq, []).append(i)

#             for uq, indices in query_pair_indices.items():
#                 documents = [all_pairs[i][1] for i in indices]
#                 reranking = voyage_client.rerank(
#                     query=uq,
#                     documents=documents,
#                     model=VOYAGE_RERANK_MODEL,
#                 )
#                 # results are returned with each result's original index into `documents`
#                 for result in reranking.results:
#                     scores[indices[result.index]] = result.relevance_score
#         else:
#             scores = []
